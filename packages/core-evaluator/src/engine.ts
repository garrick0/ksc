...
export interface Evaluator<M = Record<string, unknown>, P extends Record<string, unknown> = Record<string, unknown>> {
...
/**
 * Create an evaluator from an EvaluationTarget — the runtime counterpart to CodegenTarget.
 *
 * Convenience factory that unpacks the target's AnalysisProjections into the
 * EvaluatorConfig shape, avoiding manual destructuring at composition roots.
 */
export function createEvaluatorFromTarget<K extends string = string, M = Record<string, unknown>, P extends Record<string, unknown> = Record<string, unknown>>(target: EvaluationTarget<K, M, P>): Evaluator<M, P> {
