...
export const eq_noDupeKeysViolation_ObjectLiteralExpression = withDeps([],
  function eq_noDupeKeysViolation_ObjectLiteralExpression(
    ...
    const properties = ctx.node.properties as KSNodeBase[] | undefined;
    ...
    for (const prop of properties) {
      ...
      const nameNode = prop.name as KSNodeBase | undefined;
      ...
      if (nameNode.kind === 'Identifier') {
        key = (nameNode as KSIdentifier).escapedText;
...
function checkMaxParams(ctx: Ctx): EslintEquivDiagnostic | null {
  const params = (ctx.node as KSNode).parameters as KSNodeBase[] | undefined;
...
export const eq_noEmptyInterfaceViolation_InterfaceDeclaration = withDeps([],
  function eq_noEmptyInterfaceViolation_InterfaceDeclaration(
    ...
    const members = ctx.node.members as KSNodeBase[] | undefined;
    if (members && members.length > 0) return null;

    const heritageClauses = ctx.node.heritageClauses as KSNodeBase[] | undefined;
    ...
    for (const clause of heritageClauses) {
      if (clause.token === 'extends') {
        extendsTypeCount += (clause.types as unknown[] | undefined)?.length ?? 0;
      }
    }
...
export const eq_noDuplicateImportsViolation_CompilationUnit = withDeps([],
  function eq_noDuplicateImportsViolation_CompilationUnit(
    ...
    for (const child of ctx.children) {
      ...
      const moduleSpec = (child.node as unknown as KSImportDeclaration).moduleSpecifier;
