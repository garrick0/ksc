...
export const eq_noConsoleViolation_CallExpression = withDeps([],
  function eq_noConsoleViolation_CallExpression(
    ...
    const obj = (expr as KSPropertyAccessExpression).expression;
    if (!obj || obj.kind !== 'Identifier') return null;
    if ((obj as KSIdentifier).escapedText !== 'console') return null;
...
export const eq_noEvalViolation_CallExpression = withDeps([],
  function eq_noEvalViolation_CallExpression(
    ...
    if ((expr as KSIdentifier).escapedText !== 'eval') return null;
...
export const eq_noNewWrappersViolation_NewExpression = withDeps([],
  function eq_noNewWrappersViolation_NewExpression(
    ...
    const expr = ctx.node.expression as KSNodeBase | undefined;
    if (!expr || expr.kind !== 'Identifier') return null;
    const name = (expr as KSIdentifier).escapedText;
...
function isAssignmentExpression(node: KSNode): boolean {
  ...
  const opKind = (node as KSBinaryExpression).operatorToken?.kind;
...
function expressionText(node: KSNode): string {
  ...
  if (node.kind === 'Identifier') return (node as KSIdentifier).escapedText ?? '';
  if (node.kind === 'StringLiteral') return `"${(node as KSStringLiteral).value}"`;
  if (node.kind === 'NumericLiteral') return String(node.value);
...
function nodesStructurallyEqual(a: Ctx, b: Ctx): boolean {
  ...
  if (a.children.length === 0) {
    if (a.node.kind === 'Identifier') {
      return (a.node as unknown as KSIdentifier).escapedText === (b.node as unknown as KSIdentifier).escapedText;
    }
    if ('value' in a.node && 'value' in b.node) {
      return (a.node as Record<string, unknown>).value === (b.node as Record<string, unknown>).value;
    }
    return true;
  }
...
function isUselessCatch(catchNode: KSNodeBase): boolean {
  const varDecl = catchNode.variableDeclaration as KSNodeBase | undefined;
  if (!varDecl) return false;

  const paramName = varDecl.name as KSNodeBase | undefined;
  if (!paramName || paramName.kind !== 'Identifier') return false;
  const catchVarName = (paramName as KSIdentifier).escapedText;

  const block = catchNode.block as KSNodeBase | undefined;
  if (!block) return false;

  const stmts = block.statements as KSNodeBase[] | undefined;
  if (!stmts || stmts.length !== 1) return false;

  const stmt = stmts[0];
  if (stmt.kind !== 'ThrowStatement') return false;

  const throwExpr = stmt.expression as KSNodeBase | undefined;
  if (!throwExpr || throwExpr.kind !== 'Identifier') return false;
  return (throwExpr as KSIdentifier).escapedText === catchVarName;
}
...
export const eq_noUselessCatchViolation_TryStatement = withDeps([],
  function eq_noUselessCatchViolation_TryStatement(
    ...
    const catchClause = (ctx.node as KSNode).catchClause as KSNodeBase | undefined;
...
function hasEmptyBody(ctx: Ctx): boolean {
  ...
  const stmts = (bodyChild.node as unknown as KSBlock).statements;
...
export const eq_useIsNanViolation_BinaryExpression = withDeps([],
  function eq_useIsNanViolation_BinaryExpression(
    ...
    const leftIsNaN = ctx.node.left.kind === 'Identifier'
      && (ctx.node.left as KSIdentifier).escapedText === 'NaN';
    const rightIsNaN = ctx.node.right.kind === 'Identifier'
      && (ctx.node.right as KSIdentifier).escapedText === 'NaN';
...
export const eq_noSparseArraysViolation_ArrayLiteralExpression = withDeps([],
  function eq_noSparseArraysViolation_ArrayLiteralExpression(
    ...
    const elements = (ctx.node as KSNode).elements as KSNodeBase[] | undefined;
    if (!elements) return null;
    const hasSparse = elements.some((el) => el.kind === 'OmittedExpression');
    if (hasSparse) {
