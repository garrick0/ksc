...
export function getLocalCount(ctx: ConvertContext, node: ts.Node): number {
  ...
  return (node as unknown as Record<string, unknown>).locals
    ? ((node as unknown as Record<string, unknown>).locals as Map<unknown, unknown>).size
    : 0;
}
...
export function isNodeExported(node: ts.Node): boolean {
  const mods = (node as unknown as { modifiers?: ts.NodeArray<ts.ModifierLike> }).modifiers;
  if (mods) {
...
export function extractJSDocComment(n: unknown): string {
  const node = n as Record<string, unknown>;
  if (!node.comment) return '';
  if (typeof node.comment === 'string') return node.comment;
  return '';
}
