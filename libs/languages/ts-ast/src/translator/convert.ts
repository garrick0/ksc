...
import type { FieldDescShape, AstTranslatorPort } from '@ksc/grammar';
...
const NODES_WITH_TYPE_STRING = new Set<string>();
for (const [kind, def] of Object.entries(NODES)) {
  if ('typeString' in def.fields) NODES_WITH_TYPE_STRING.add(kind);
}
...
function extractField(
  ...
  if (fname === 'typeString' && NODES_WITH_TYPE_STRING.has(kind)) {
  ...
  if (fname === 'comment' && field.tag === 'prop' && field.propType === 'string' && JSDOC_MEMBERS.has(kind)) {
...
/**
 * TS AST → KS AST converter.
 *
 * Implements the convert() method of AstTranslatorPort.
 */
export function convertTSAST(tsProgram: ts.Program, depth: AnalysisDepth = 'check'): KSTree {
