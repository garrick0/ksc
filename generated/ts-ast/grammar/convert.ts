...
import { hasSymFlag, checkIsDefinitionSite, isImportReference, getResolvedFileName, isNodeExported, getLocalCount, getTypeString, getResolvedModulePath, getImportModuleSpecifier, extractJSDocComment, getDeclarationKind, prefixUnaryOperatorMap, postfixUnaryOperatorMap, typeOperatorMap, heritageTokenMap, metaPropertyKeywordMap } from '../../../specs/ts-ast/grammar/convert-helpers.js';
...
register(ts.SyntaxKind.Identifier, (node, sf, children, pos, end, text) => {
  ...
  return {
    ...
    resolvesToImport: isImportReference(_ctx, node),
    isDefinitionSite: checkIsDefinitionSite(node),
    resolvedFileName: getResolvedFileName(_ctx, node),
    symIsVariable: hasSymFlag(_ctx, node, ts.SymbolFlags.Variable),
    symIsFunctionScopedVariable: hasSymFlag(_ctx, node, ts.SymbolFlags.FunctionScopedVariable),
    symIsBlockScopedVariable: hasSymFlag(_ctx, node, ts.SymbolFlags.BlockScopedVariable),
    symIsFunction: hasSymFlag(_ctx, node, ts.SymbolFlags.Function),
    symIsClass: hasSymFlag(_ctx, node, ts.SymbolFlags.Class),
    symIsInterface: hasSymFlag(_ctx, node, ts.SymbolFlags.Interface),
    symIsTypeAlias: hasSymFlag(_ctx, node, ts.SymbolFlags.TypeAlias),
    symIsAlias: hasSymFlag(_ctx, node, ts.SymbolFlags.Alias),
    symIsProperty: hasSymFlag(_ctx, node, ts.SymbolFlags.Property),
    symIsMethod: hasSymFlag(_ctx, node, ts.SymbolFlags.Method),
    symIsEnum: hasSymFlag(_ctx, node, ts.SymbolFlags.Enum),
    symIsEnumMember: hasSymFlag(_ctx, node, ts.SymbolFlags.EnumMember),
    symIsNamespace: hasSymFlag(_ctx, node, ts.SymbolFlags.NamespaceModule),
    symIsExportValue: hasSymFlag(_ctx, node, ts.SymbolFlags.ExportValue),
    symIsType: hasSymFlag(_ctx, node, ts.SymbolFlags.Type),
    symIsValue: hasSymFlag(_ctx, node, ts.SymbolFlags.Value),
    importModuleSpecifier: getImportModuleSpecifier(_ctx, node),
    typeString: getTypeString(_ctx, node),
...
register(ts.SyntaxKind.FunctionDeclaration, (node, sf, children, pos, end, text) => {
  ...
  return {
    ...
    isExported: isNodeExported(node),
    localCount: getLocalCount(_ctx, node),
    typeString: getTypeString(_ctx, node),
    pos, end, text, children,
  } as KS.KSNode;
});
...
register(ts.SyntaxKind.ImportDeclaration, (node, sf, children, pos, end, text) => {
  ...
  return {
    ...
    modifiers: findChildrenOf(children, n.modifiers),
    resolvedModulePath: getResolvedModulePath(_ctx, node as any),
    pos, end, text, children,
  } as KS.KSNode;
});
...
export function buildKSTree(tsProgram: ts.Program, depth: AnalysisDepth = 'check'): KSTree {
