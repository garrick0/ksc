...
import type { KSNode, KSCommentRange, KSProgram, KSCompilationUnit, KSStatement } from '../grammar/index.js';
import type { FieldDescShape, Frontend } from '../../../grammar/index.js';
import { NODES, fieldDefs } from '../grammar/index.js';
import {
...
interface ConvertContext extends BaseConvertContext {
...
// ═══════════════════════════════════════════════════════════════════════
// Custom field extractors — TS-specific transformations
// ═══════════════════════════════════════════════════════════════════════

type FieldExtractorFn = (ctx: ConvertContext, n: Record<string, unknown>, node: ts.Node, children: KSNode[]) => unknown;

/**
 * Per-kind custom field extractors.
 * Maps [nodeKind][fieldName] → extractor function.
 * Fields not listed here are handled generically from the schema.
 */
const CUSTOM_EXTRACTORS: Record<string, Record<string, FieldExtractorFn>> = {
  Identifier: {
...
// ═══════════════════════════════════════════════════════════════════════
// Public API — Adapter: Frontend<ts.Program, KSProgram>
// ═══════════════════════════════════════════════════════════════════════

function buildKSTree(tsProgram: ts.Program, depth: AnalysisDepth = 'check'): KSTree {
  const checker = depth !== 'parse' ? tsProgram.getTypeChecker() : undefined;
  const ctx: ConvertContext = { checker, depth, tsToKs: new WeakMap() };
...
/** Adapter: Frontend port — TS AST → KS AST converter. */
