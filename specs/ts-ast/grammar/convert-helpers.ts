/**
 * TypeScript AST extractor helpers — language-specific functions for convert.ts.
 *
 ...
 * grammar/compile.ts fully generic.
 */
...
export function getResolvedFileName(ctx: ConvertContext, node: ts.Node): string {
  if (!ctx.checker || !ts.isIdentifier(node)) return '';
  const sym = ctx.checker.getSymbolAtLocation(node);
...
export function getTypeString(ctx: ConvertContext, node: ts.Node): string {
  if (!ctx.checker || ctx.depth !== 'check') return '';
  try {
...
export function isImportReference(ctx: ConvertContext, node: ts.Node): boolean {
  if (!ctx.checker || !ts.isIdentifier(node)) return false;
  const sym = ctx.checker.getSymbolAtLocation(node);
  if (!sym) return false;
