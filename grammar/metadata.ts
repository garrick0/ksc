 ...
 *
 * All functions take a schema as input. No module-level state, no imports from specs/.
 * Reusable for any grammar (TS AST, mock, future).
 */

import type { FieldDef, ChildFieldDef, PropFieldDef } from './ports.js';
import type { FieldDescShape, NodeDefShape, SumTypeDefShape } from './schema-shapes.js';
...
): (node: import('./ports.js').ASTNode) => boolean {
  const set = new Set(memberKinds);
  return (node: import('./ports.js').ASTNode) => set.has(node.kind);
}
