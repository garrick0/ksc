...
export function useD3Tree(containerRef: React.RefObject<HTMLDivElement | null>, opts: Opts) {
  ...
  useEffect(() => {
    ...
    function update(source: D3Node) {
      ...
      nodeEnter.on('mousemove', (e: MouseEvent, d: D3Node) => {
        if (d.data.tooltip) opts.showTooltip(e, d.data.tooltip);
      });
      nodeEnter.on('mouseleave', () => opts.hideTooltip());
    ...
    function handleAction(action: { type: string; payload: unknown }) {
      ...
      opts.dispatch({ type: 'OPEN_DETAIL', detailType: payload.detailType, payload: payload.payload });
    }
