...
import { pivotToAttrCentric, compileAnalysis, code, withDeps } from '@kindscript/core-codegen';
...
describe('codegen-time validation', () => {
  ...
  it('throws for unknown fileContainerKind', () => {
    const badRootGrammar: Grammar = { ...gAB, fileContainerKind: 'NonexistentRoot' };
    const spec: AnalysisDecl = {
      attrs: [
    ...
    expect(() => compileAnalysis(badRootGrammar, spec)).toThrow("fileContainerKind 'NonexistentRoot' is not a valid kind");
  });
