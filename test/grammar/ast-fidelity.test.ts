...
import { type AnalysisDepth } from '@ksc/language-ts-ast/translator/convert.js';
import type { KSNode, KSCompilationUnit } from '@ksc/language-ts-ast/grammar/index.js';
import { fieldDefs, allKinds } from '@ksc/language-ts-ast/grammar/index.js';
...
describe('integration — KSC ↔ TS AST fidelity (check depth)', () => {
  ...
  it('exercises sufficient breadth: >= 150 kinds, >= 1000 nodes, and fields are not trivially all-false', () => {
...
describe('integration — depth degradation', () => {
  it('parse depth: structural integrity holds, checker-dependent fields have defaults', () => {
    ...
    function walk(node: KSNode) {
      ...
      if ('typeString' in ks) expect(ks.typeString).toBe('');
      if ('localCount' in ks) expect(ks.localCount).toBe(0);
  ...
  it('bind depth: structural integrity + sym* flags hold, typeString is empty', () => {
