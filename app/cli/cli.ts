...
import { createProgram } from '../user-api/lib/program.js';
import type { KindScriptConfig } from '../user-api/lib/config.js';
...
export function parseArgv(argv: string[]): CLIOptions {
  ...
  function parseDepth(val: string): 'parse' | 'bind' | 'check' {
    if (val === 'parse' || val === 'bind' || val === 'check') return val;
...
async function loadConfig(configPath: string): Promise<KindScriptConfig> {
  ...
  if (config.analysisDepth !== undefined &&
      config.analysisDepth !== 'parse' &&
      config.analysisDepth !== 'bind' &&
      config.analysisDepth !== 'check') {
    throw new Error(`Invalid analysisDepth '${config.analysisDepth}' in config. Must be 'parse', 'bind', or 'check'.`);
  }
